# Write a program that outputs the lyricss for "Ninety-nine Bottles of Beer on the Wall"
...
class BeerSong
  ...
  def lyrics(n)
    if n.zero?
      ...
      puts "#{transl n} #{bottle n} of beer on the wall,"       ,
           "#{transl n} #{bottle n} of beer,"                    ,
           "Take one down, pass it around,"                        ,
           "#{transl n - 1} #{bottle n-1} of beer on the wall."
    end
  end
  ...
  def transl(n)
    if 0 <= n && n <= 19
      ...
      "#{transl n/10*10}-#{transl n % 10}".downcase
    end.capitalize
  end
end
